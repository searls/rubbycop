...
module RubbyCop
  ...
  class ConfigLoader
    DOTFILE = '.rubbycop.yml'.freeze
    RUBOCOP_HOME = File.realpath(File.join(File.dirname(__FILE__), '..', '..'))
    DEFAULT_FILE = File.join(RUBOCOP_HOME, 'config', 'default.yml')
    AUTO_GENERATED_FILE = '.rubbycop_todo.yml'.freeze
    ...
    class << self
      ...
      # Returns the path of .rubbycop.yml searching upwards in the
      # directory structure starting at the given directory where the
      # inspected file is. If no .rubbycop.yml is found there, the
      # user's home directory is checked. If there's no .rubbycop.yml
      ...
      def yaml_safe_load(yaml_code, filename)
        if YAML.respond_to?(:safe_load) # Ruby 2.1+
          ...
          YAML.load(yaml_code, filename) # rubbycop:disable Security/YAMLLoad
        end
      end

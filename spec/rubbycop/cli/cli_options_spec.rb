...
describe RubbyCop::CLI, :isolated_environment do
  ...
  describe '--parallel' do
    if RubbyCop::Platform.windows?
      ...
      context 'combined with AllCops:UseCache:false' do
        before do
          create_file('.rubbycop.yml', ['AllCops:',
                                       '  UseCache: false'])
        end
  ...
  describe '--list-target-files' do
    ...
    context 'when there are some files' do
      before do
        ...
        create_file('Gemfile', <<-END.strip_indent)
  ...
  describe '--only' do
    context 'when one cop is given' do
      it 'runs just one cop' do
        ...
        create_file('example.rb', ['# rubbycop:disable LineLength',
                                   'if x== 0 ',
                                   "\ty",
                                   'end'])
      ...
      it 'accepts cop names from plugins' do
        create_file('.rubbycop.yml', <<-END.strip_indent)
          require: rubbycop_ext

          Style/SomeCop:
            Description: Something
            Enabled: true
        END
        create_file('rubbycop_ext.rb', <<-END.strip_indent)
          module RubbyCop
            module Cop
              module Style
                class SomeCop < Cop
      ...
      it 'enables the given cop' do
        create_file('example.rb',
                    ['x = 0 ',
                     ...
                     '# rubbycop:disable Layout/TrailingWhitespace',
                     'y = 1  '])
  ...
  describe '-D/--display-cop-names' do
    it 'shows cop names' do
      create_file('example1.rb', 'puts 0 # rubbycop:disable NumericLiterals ')
      file = abs('example1.rb')
      ...
      expect($stdout.string)
        .to eq(["#{file}:1:8: W: Lint/UnneededDisable: Unnecessary " \
                ...
                "#{file}:1:42: C: Layout/TrailingWhitespace: Trailing " \
                'whitespace detected.',
                ''].join("\n"))
    end
  end
  ...
  describe '-S/--display-style-guide' do
    it 'shows style guide entry' do
      ...
      file = abs('example1.rb')
      url =
        'https://github.com/searls/ruby-style-guide#no-trailing-whitespace'
    ...
    it 'shows style guide and reference entries' do
      ...
      file = abs('example1.rb')
      style_guide_link = 'https://github.com/searls/ruby-style-guide' \
                         '#instance-vars'
  ...
  describe '--show-cops' do
    shared_examples(:prints_config) do
      it 'prints the current configuration' do
        ...
        printed_config = YAML.load(out.join) # rubbycop:disable Security/YAMLLoad
    ...
    describe 'builtin formatters' do
      ...
      context 'when clang format is specified' do
        it 'outputs with clang format' do
          ...
          END
          create_file('example3.rb', <<-END.strip_indent)
            def badName
              if something
                test
                end
  ...
  describe 'with --auto-correct and disabled offense' do
    ...
    it 'succeeds when there is only a disabled offense' do
      create_file(target_file, <<-END.strip_indent)
        def f
         x # rubbycop:disable Layout/IndentationWidth
        end
      END

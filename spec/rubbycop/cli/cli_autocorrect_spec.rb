...
describe RubbyCop::CLI, :isolated_environment do
  ...
  describe 'trailing comma cops' do
    ...
    before do
      create_file('.rubbycop.yml', YAML.dump(config))
    end
  ...
  it 'corrects UnneededDisable offenses' do
    source = <<-END.strip_indent
      class A
        ...
        def func
          x = foo # rubbycop:disable Lint/UselessAssignment,Style/For
    ...
    expect($stdout.string).to eq(<<-END.strip_indent)
      ...
      W:  4: 55: [Corrected] Unnecessary disabling of Style/For.
    ...
    corrected = <<-END.strip_indent
      class A
        def func
          x = foo # rubbycop:disable Lint/UselessAssignment
          # rubbycop:disable all
          @@bar = "3"
        end
      end

...
describe RubbyCop::RemoteConfig do
  ...
  let(:remote_config_url) { 'http://example.com/rubbycop.yml' }
  let(:base_dir) { '.' }
  let(:cached_file_name) { '.rubbycop-http---example-com-rubbycop-yml' }
  ...
  describe '.file' do
    ...
    context 'when the remote URL responds with redirect' do
      let(:new_location) { 'http://cdn.example.com/rubbycop.yml' }

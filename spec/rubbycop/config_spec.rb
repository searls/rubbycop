...
describe RubbyCop::Config do
  ...
  let(:hash) { {} }
  let(:loaded_path) { 'example/.rubbycop.yml' }
      ...
      before do
        create_file(configuration_path, <<-END.strip_indent)
          InvalidProperty:
            Enabled: true
        END
        stub_const('RubbyCop::ConfigLoader::RUBOCOP_HOME', rubbycop_home_path)
      end

      let(:rubbycop_home_path) { File.realpath('.') }
      let(:configuration_path) { 'config/.rubbycop.yml' }
  ...
  describe '#file_to_include?' do
    ...
    let(:loaded_path) { '/home/foo/project/.rubbycop.yml' }

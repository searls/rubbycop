...
describe RubbyCop::ConfigLoader do
  ...
  describe '.configuration_file_for', :isolated_environment do
    ...
    context 'when no config file exists in ancestor directories' do
      ...
      context 'but a config file exists in home directory' do
        before { create_file('~/.rubbycop.yml', '') }

        it 'returns the path to the file in home directory' do
          expect(configuration_file_for).to end_with('home/.rubbycop.yml')
        end
      end
    ...
    context 'when a config file exists in the parent directory' do
      ...
      it 'returns the path to that configuration file' do
        expect(configuration_file_for).to end_with('work/.rubbycop.yml')
      end
    end
    ...
    context 'when multiple config files exist in ancestor directories' do
      ...
      before do
        create_file('dir/example.rb', '')
        create_file('dir/.rubbycop.yml', '')
        create_file('.rubbycop.yml', '')
      end

      it 'prefers closer config file' do
        expect(configuration_file_for).to end_with('dir/.rubbycop.yml')
      end
    end
  end
  ...
  describe '.configuration_from_file', :isolated_environment do
    ...
    context 'when multiple config files exist in ancestor directories' do
      let(:file_path) { 'dir/.rubbycop.yml' }
    ...
    context 'when a file inherits from a parent file' do
      ...
      before do
        ...
        create_file(file_path, ['inherit_from: ../.rubbycop.yml'])
      end
    ...
    context 'when a file inherits from a sibling file' do
      ...
      before do
        create_file('src/.rubbycop.yml', <<-END.strip_indent)
          AllCops:
            Exclude:
              - vendor/**
        END

        create_file(file_path, ['inherit_from: ../src/.rubbycop.yml'])
      end
    ...
    context 'when a third party require defines a new gem' do
      before do
        ...
        create_file('.rubbycop_with_require.yml', <<-END.strip_indent)
      ...
      it 'does not emit a warning' do
        aggregate_failures('loads requires before resolving namespace') do
          expect { described_class.configuration_from_file('.rubbycop.yml') }
            .to output(
              a_string_including(
                '.rubbycop.yml: Custom/FilePath has the ' \
                "wrong namespace - should be Rails\n"
              )
            ).to_stderr

          expect do
            described_class.configuration_from_file('.rubbycop_with_require.yml')
    ...
    context 'when a file inherits from a parent and grandparent file' do
      let(:file_path) { 'dir/subdir/.rubbycop.yml' }
    ...
    context 'when a file inherits from two configurations' do
      ...
      it 'returns values from the last one when possible' do
        expected = { 'Enabled' => true,        # overridden in .rubbycop.yml
        ...
        config =
          default_config.merge(
            'Metrics/LineLength' => {
              ...
              'AllowHeredoc' => false,  # overridden in rubbycop.yml
              'AllowURI' => true,
              'URISchemes' => %w[http https],
              'IgnoreCopDirectives' => false,
              'IgnoredPatterns' => []
    ...
    context 'when a file inherits from an unknown gem' do
      ...
      before do
        create_file(file_path, <<-END.strip_indent)
          inherit_gem:
            not_a_real_gem: config/rubbycop.yml
        END
      end
    ...
    context 'when a file inherits from the rubbycop gem' do
      let(:file_path) { '.rubbycop.yml' }

      before do
        create_file(file_path, <<-END.strip_indent)
          inherit_gem:
            rubbycop: config/default.yml
        END
      end
    ...
    context 'when a file inherits from a known gem' do
      ...
      before do
        create_file('gemone/config/rubbycop.yml', <<-END.strip_indent)
          Metrics/MethodLength:
            Enabled: false
            Max: 200
            CountComments: false
      ...
      it 'returns values from the gem config with local overrides' do
        ...
        expected = { 'Enabled' => true,        # gemtwo/config/default.yml
                     ...
                     'AllowURI' => false }     # overridden in .rubbycop.yml
    ...
    context 'when a file inherits from a url' do
      let(:file_path) { '.rubbycop.yml' }
      let(:cache_file) { '.rubbycop-http---example-com-rubbycop-yml' }

      before do
        stub_request(:get, /example.com/)
          .to_return(status: 200, body: "Style/Encoding:\n    Enabled: true")

        create_file(file_path, ['inherit_from: http://example.com/rubbycop.yml'])
      end
  ...
  describe '.load_file', :isolated_environment do
    ...
    let(:configuration_path) { '.rubbycop.yml' }
    ...
    it 'fails with a TypeError when loading a malformed configuration file' do
      ...
      expect { load_file }.to raise_error(
        TypeError, /^Malformed configuration in .*\.rubbycop\.yml$/
      )
    end
      ...
      context 'when it is fully required' do
        it 'de-serializes Regexp class' do
          in_its_own_process_with('safe_yaml') do
            configuration = described_class.load_file('.rubbycop.yml')
  ...
  describe 'configuration for CharacterLiteral', :isolated_environment do
    ...
    context 'when .rubbycop.yml inherits from a file with a name starting ' \
            'with .rubbycop' do
      before do
        create_file('test/.rubbycop_rules.yml', <<-END.strip_indent)
          Style/CharacterLiteral:
            Exclude:
              - blargh/blah.rb
        END
        create_file('test/.rubbycop.yml', 'inherit_from: .rubbycop_rules.yml')
      end
  ...
  describe 'when a requirement is defined', :isolated_environment do
    ...
    before do
      create_file('.rubbycop.yml', ['require:', "  - #{required_file_path}"])
    ...
    it 'uses paths relative to the .rubbycop.yml, not cwd' do
      config_path = described_class.configuration_file_for('.')
      Dir.chdir '..' do
        described_class.configuration_from_file(config_path)
        expect(defined?(MyClass)).to be_truthy

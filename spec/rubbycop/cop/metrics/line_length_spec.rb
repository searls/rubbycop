...
describe RubbyCop::Cop::Metrics::LineLength, :config do
  ...
  context 'when AllowURI option is enabled' do
    ...
    context 'and all the excessive characters are part of an URL' do
      ...
      let(:source) { <<-END }
        ...
        # See: https://github.com/searls/rubbycop/commit/3b48d8bdf5b1c2e05e35061837309890f04ab08c
      END
    ...
    context 'and the excessive characters include part of an URL ' \
            'and another word' do
      let(:source) { <<-END }
        # See: https://github.com/searls/rubbycop/commit/3b48d8bdf5b1c2e05e35061837309890f04ab08c and
        #   http://google.com/
      END
  ...
  context 'when IgnoreCopDirectives is disabled' do
    ...
    context 'and the source is too long and has a trailing cop directive' do
      let(:excess_with_directive) { 'b # rubbycop:disable Metrics/AbcSize' }
  ...
  context 'when IgnoreCopDirectives is enabled' do
    ...
    context 'and the Rubbycop directive causes an excessive line length' do
      ...
      context 'and has explanatory text' do
        let(:source) { <<-END }
          def method_definition_that_is_just_under_the_line_length_limit(foo) # rubbycop:disable Metrics/AbcSize inherently complex!
            # complex
          end
        END
    ...
    context 'and the source is too long' do
      let(:source) { 'a' * 80 + 'bcd' + ' # rubbycop:enable Style/ClassVars' }
      ...
      context 'and the source contains non-directive # as comment' do
        let(:source) { <<-END }
          aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa # bbbbbbbbbbbbbb # rubbycop:enable Style/ClassVars'
        END
      ...
      context 'and the source contains non-directive #s as non-comment' do
        let(:source) { <<-END }
          LARGE_DATA_STRING_PATTERN = %r{\A([A-Za-z0-9\+\/#]*\={0,2})#([A-Za-z0-9\+\/#]*\={0,2})#([A-Za-z0-9\+\/#]*\={0,2})\z} # rubbycop:disable LineLength
        END

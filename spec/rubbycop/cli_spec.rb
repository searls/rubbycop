...
describe RubbyCop::CLI, :isolated_environment do
  ...
  describe 'rubbycop:disable comment' do
    it 'can disable all cops in a code section' do
      src = ['# rubbycop:disable all',
             '#' * 90,
             'x(123456)',
             'y("123")',
             'def func',
    ...
    context 'when --auto-correct is given' do
      it 'does not trigger UnneededDisable due to lines moving around' do
        src = ['a = 1 # rubbycop:disable Lint/UselessAssignment']
        create_file('example.rb', src)
        create_file('.rubbycop.yml', <<-END.strip_indent)
        ...
        expect(IO.read('example.rb'))
          .to eq(<<-END.strip_indent)
            ...
            a = 1 # rubbycop:disable Lint/UselessAssignment
          END
      end
    end
    ...
    it 'can disable selected cops in a code section' do
      create_file('example.rb',
                  ['# rubbycop:disable Style/LineLength,' \
    ...
    it 'can disable all cops on a single line' do
      create_file('example.rb', 'y("123", 123456) # rubbycop:disable all')
    ...
    it 'can disable selected cops on a single line' do
      create_file('example.rb',
                  ['a' * 90 + ' # rubbycop:disable Metrics/LineLength',
                   '#' * 95,
                   'y("123", 123456) # rubbycop:disable Style/StringLiterals,' \
    ...
    context 'without using namespace' do
      it 'can disable selected cops on a single line' do
        create_file('example.rb',
                    ['a' * 90 + ' # rubbycop:disable LineLength',
                     '#' * 95,
                     'y("123") # rubbycop:disable StringLiterals'])
        expect(cli.run(['--format', 'emacs', 'example.rb'])).to eq(1)
        expect($stdout.string)
          .to eq(<<-END.strip_indent)
            #{abs('example.rb')}:2:81: C: Line is too long. [95/80]
    ...
    context 'when not necessary' do
      it 'causes an offense to be reported' do
        create_file('example.rb',
                    ['#' * 95,
                     ...
                     'a' * 10 + ' # rubbycop:disable LineLength,ClassLength',
                     'y(123) # rubbycop:disable all'])
        expect(cli.run(['--format', 'emacs', 'example.rb'])).to eq(1)
      ...
      context 'and there are no other offenses' do
        it 'exits with error code' do
          create_file('example.rb', 'a' * 10 + ' # rubbycop:disable LineLength')
          expect(cli.run(['example.rb'])).to eq(1)
        end
      end
          ...
          it 'does not report UnneededDisable offenses' do
            create_file('example.rb',
                        ['#' * 95,
                         ...
                         'a' * 10 + ' # rubbycop:disable LineLength,ClassLength',
                         'y(123) # rubbycop:disable all'])
            create_file('.rubbycop.yml', config)
      ...
      include_examples 'UnneededDisable not run',
                       'individually disabled', <<-END.strip_indent
        Lint/UnneededDisable:
          Enabled: false
      END
      include_examples 'UnneededDisable not run',
                       'individually excluded', <<-END.strip_indent
        Lint/UnneededDisable:
          Exclude:
            - example.rb
      END
      include_examples 'UnneededDisable not run',
                       'disabled through department', <<-END.strip_indent
        Lint:
          Enabled: false
      END
    end
    ...
    context 'via the config' do
      ...
      describe 'AllCops/DisabledByDefault' do
        ...
        context 'when it is true' do
          ...
          it 'runs only the cop configured in .rubbycop.yml' do
            expect(cli.run(['--format', 'offenses', 'example.rb'])).to eq(1)
  ...
  describe 'rails cops' do
    ...
    describe 'enabling/disabling' do
      ...
      it 'with configuration option true in one dir runs rails cops there' do
        ...
        create_file('dir1/.rubbycop.yml', <<-END.strip_indent)
          Rails:
            Enabled: true
        ...
        create_file('dir2/.rubbycop.yml', <<-END.strip_indent)
          Rails:
            Enabled: false
    ...
    describe 'including/excluding' do
      it 'honors Exclude settings in .rubbycop_todo.yml one level up' do
        create_file('lib/example.rb', 'puts %x(ls)')
        create_file('.rubbycop.yml', 'inherit_from: .rubbycop_todo.yml')
        create_file('.rubbycop_todo.yml', <<-END.strip_indent)
      ...
      it 'includes some directories by default' do
        ...
        # The .rubbycop.yml file inherits from default.yml where the Include
        # config parameter is set for the rails cops. The paths are interpreted
        # as relative to dir1 because .rubbycop.yml is placed there.
        create_file('dir1/.rubbycop.yml', <<-END.strip_indent)
          Rails:
            Enabled: true
          ...
          Rails/ReadWriteAttribute:
            Exclude:
              - "**/example2.rb"
        END
        # No .rubbycop.yml file in dir2 means that the paths from default.yml
  ...
  describe 'configuration from file' do
    ...
    it 'allows the default configuration file as the -c argument' do
      create_file('example.rb', ['x = 0', 'puts x'])
      create_file('.rubbycop.yml', [])

      expect(cli.run(%w[--format simple -c .rubbycop.yml])).to eq(0)
      expect($stdout.string)
        .to eq(['', '1 file inspected, no offenses detected',
                ''].join("\n"))
    end
    ...
    context 'when --force-default-config option is specified' do
      ...
      context 'when config file specified with -c' do
        let(:args) do
          %w[--format simple --force-default-config -c .rubbycop.yml]
        end
        include_examples 'ignores config file'
      end
    end
    ...
    it 'displays cop names if DisplayCopNames is true' do
      ...
      create_file('dir/.rubbycop.yml', <<-END.strip_indent)
        AllCops:
    ...
    it 'displays style guide URLs if DisplayStyleGuide is true' do
      ...
      url = 'https://github.com/searls/ruby-style-guide#no-trailing-whitespace'
    ...
    it 'only reads configuration in explicitly included hidden directories' do
      ...
      create_file('.hidden/.rubbycop.yml', <<-END.strip_indent)
        SymbolName:
          Enabled: false
      END
      ...
      create_file('.other/.rubbycop.yml', <<-END.strip_indent)
        Layout/SpaceAroundOperators:
          Enabled: false
      END
      create_file('.rubbycop.yml', <<-END.strip_indent)
    ...
    it 'does not read files in excluded list', broken: :rbx do
      ...
      create_file('example/.rubbycop.yml', <<-END.strip_indent)
        AllCops:
          Exclude:
            - ignored/**
      END
    ...
    it 'can be configured with option to disable a certain error' do
      ...
      expect(cli.run(['--format', 'simple',
                      '-c', 'rubbycop.yml', 'example1.rb'])).to eq(1)
      expect($stdout.string)
        .to eq(<<-END.strip_indent)
          == example1.rb ==
      ...
      create_file('rubbycop.yml', <<-END.strip_indent)
        Style/PercentLiteralDelimiters:
          PreferredDelimiters:
            ...
            '%W': '[]'
      END
      cli.run(['--format', 'simple', '-c', 'rubbycop.yml', 'example1.rb'])
      expect($stdout.string)
        .to eq(['== example1.rb ==',
    ...
    it 'can be configured with project config to disable a certain error' do
      ...
      create_file('example_src/.rubbycop.yml', <<-END.strip_indent)
        Style/Encoding:
          Enabled: false
    ...
    it 'can have different config files in different directories' do
      ...
      create_file('example/src/.rubbycop.yml', <<-END.strip_indent)
        Metrics/LineLength:
          Enabled: true
          Max: 100
      END
    ...
    it 'prefers a config file in ancestor directory to another in home' do
      ...
      create_file('example_src/.rubbycop.yml', <<-END.strip_indent)
        Metrics/LineLength:
          ...
          Max: 100
      END
      create_file("#{Dir.home}/.rubbycop.yml", <<-END.strip_indent)
        Metrics/LineLength:
          Enabled: true
          Max: 80
      END
    ...
    it 'can exclude directories relative to .rubbycop.yml' do
    ...
    it 'can exclude a typical vendor directory' do
      create_file(
        'vendor/bundle/ruby/1.9.1/gems/parser-2.0.0/.rubbycop.yml',
        <<-END.strip_indent
          AllCops:
            Exclude:
              - lib/parser/lexer.rb
    ...
    # installations of, for example, rubbycop lack their .rubbycop.yml in the
    # root directory.
    it 'can exclude a vendor directory with an erroneous config file' do
      create_file('vendor/bundle/ruby/1.9.1/gems/parser-2.0.0/.rubbycop.yml',
    ...
    # Relative exclude paths in .rubbycop.yml files are relative to that file,
    ...
    it 'can exclude a vendor directory indirectly' do
      ...
      create_file('config/default.yml', <<-END.strip_indent)
        AllCops:
          Exclude:
            - vendor/**/*
      END
    ...
    it 'prints a warning for an unrecognized cop name in .rubbycop.yml' do
    ...
    it 'prints a warning for an unrecognized configuration parameter' do
      ...
      expect($stderr.string)
        .to eq(['Warning: unrecognized parameter Metrics/LineLength:Min ' \
                'found in ' + abs('example/.rubbycop.yml'),
                ''].join("\n"))
    end
    ...
    it 'works when a configuration file passed by -c specifies Exclude ' \
      ...
      cli.run(%w[--format simple -c rubbycop.yml])
      expect($stdout.string)
        .to eq(['', '0 files inspected, no offenses detected',
                ''].join("\n"))
    end
    ...
    it 'fails when a configuration file has invalid YAML syntax' do
      ...
      expect($stderr.string)
        .to match(%r{^\(\S+example/\.rubbycop\.yml\):\ (did\ not\ find\ )?
        ...
        create_file('.rubbycop.yml', <<-END.strip_indent)
          Metrics/LineLength:
            Exclude:
              - dir/example.rb
        END
        create_file('dir/.rubbycop.yml', 'inherit_from: ../.rubbycop.yml')
    ...
    context 'when configuration is taken from $HOME/.rubbycop.yml' do
      before do
        create_file("#{Dir.home}/.rubbycop.yml", <<-END.strip_indent)
          Metrics/LineLength:
            Exclude:
    ...
    it 'shows an error if the input file cannot be found' do
      begin
        ...
        expect(e.message)
          .to eq 'rubbycop: No such file or directory -- /tmp/not_a_file'
      end
    end
  end
      ...
      it 'fails with an error message' do
        ...
        expect($stderr.string.strip).to eq(
          ['Error: The `Style/TrailingComma` cop no longer exists. Please ' \
           ...
           "(obsolete configuration found in #{abs('.rubbycop.yml')}, " \
